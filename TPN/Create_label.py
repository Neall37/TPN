...
def fetch_protein_sequences(species_name, max_sequences=22000, output_file='output_sequences.fasta'):
    """
    Fetch protein sequences for a given species from NCBI.

    Parameters:
    species_name (str): The name of the species to fetch sequences for.
    ...
    """
    # Set your email (NCBI requires this for using their API)
    Entrez.email = 'swang365@jh.edu'
    Entrez.api_key = "73d91c72c4c63f211fe65c697885d413d808"
    ...
    for j in range(0, len(sequence_ids), chunk_size):
        ...
        time.sleep(0.25)
    print(len(protein_accession_list))
    #with Entrez.efetch(db="protein", id=protein_accession_list, rettype="fasta", retmode="text") as handle:
    sequences = fetch_sequences_in_batches(protein_accession_list)
    SeqIO.write(sequences, output_file, "fasta")
    # batchsize = 300
    # for i in range(0, len(protein_accession_list), batchsize):  # Batch fetching in chunks
    #     batch_ids = protein_accession_list[i:i + batchsize]
    #     time.sleep(0.2)
    #     with Entrez.efetch(db="protein", term=','.join(batch_ids), rettype="fasta", retmode="text") as handle:
    #         print('running')
...
# Function to read CSV and process each species
...
if __name__ == "__main__":
    # process_species_data('Training.csv', 'Data_h5\\training.h5')
    # process_species_data('Validation.csv', 'Data_h5\\validation.h5')
    # process_species_data('Non_label.csv', 'Data_h5\\Non_label.h5', datatype='unlabeled':)

     df = pd.read_csv('Data_final_csv/unlabel.csv')
     ...
     for i in range(5000, 22000, step_size):
         ...
         #list_of_position_list.append(positions_idx)
    #
    # merged_list = [item for sublist in list_of_position_list for item in sublist]
    #      idx_name = f"idx_test_{i}.csv"
    #      with open(idx_name, mode='w', newline='', encoding='utf-8') as file:
    #          writer = csv.writer(file)
    #          writer.writerow(['file_name', 'position_idx'])
    #          # Merge into a single list
    #          for row in positions_idx:
    #              writer.writerow(row)
    ...
    #
    # df = pd.read_csv('/home/wuyou/Desk/TBN_code/Model/Data/Data_final_csv/Eval.csv')
    # # Filter DataFrame for ProteinCount between 6000 and 7000
    # positions_idx = process_species_data(df=df, uni_length=21000,
    #                                          h5_name=f"Eval_8.h5", datatype='labeled')
    #list_of_position_list.append(positions_idx)
   #
   # merged_list = [item for sublist in list_of_position_list for item in sublist]
